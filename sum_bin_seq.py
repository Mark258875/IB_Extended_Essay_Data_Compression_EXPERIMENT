...
# ---------- robust binary dataset matcher ----------
BIN_PAT = re.compile(r"[\\/]+binary[\\/]+", re.IGNORECASE)

def is_binary_dataset(dataset: str) -> bool:
    """Heuristics to decide if a row belongs to the Binary experiment."""
    s = (dataset or "").lower()
    s_slash = s.replace("\\", "/")
    return (
        bool(BIN_PAT.search(s_slash)) or
        ("ascii01" in s) or
        ("bitpack" in s)
    )
...
def extract_distribution(dataset: str) -> str:
    """
    Build a distribution key from the dataset path:
    Examples: 'p0.01_blocks', 'p0.05_alternating', etc.
...
def read_binary_rows(csv_paths: List[Path], debug: bool = False) -> List[Dict[str, Any]]:
    ...
    if debug:
        print(f"[stats] files_loaded={files_loaded}, rows_seen={rows_seen}, "
              f"binary_rows_matched={matched_binary}, returned={len(rows)}")
        if files_loaded == 0:
            print("[hint] Check the hard-coded CSV paths in this script.")
        if rows_seen > 0 and matched_binary == 0:
            print("[hint] No 'binary' detected. Adjust is_binary_dataset() to your folder names.")
...
\centering
\caption{Binary sequential experiment: best/worst per distribution (ratio, \% over entropy, and speed).}
\label{tab:binary_sequential_best_worst}
...
def main():
    ap = argparse.ArgumentParser(
        description="Summarize Binary SEQUENTIAL experiment: best/worst ratio (with % over entropy) and speed per distribution."
    )

    # HARD-CODED PATHS for your folder layout:
    ap.add_argument(
        "--brotli-csv",
        type=Path,
        default=Path("results") / "seq" / "results_brotli.csv",
        help="Path to Brotli CSV (default: results/seq/results_brotli.csv)",
    )
    ap.add_argument(
        "--lz4-csv",
        type=Path,
        default=Path("results") / "seq" / "results_lz4.csv",
        help="Path to LZ4 CSV (default: results/seq/results_lz4.csv)",
    )
    ap.add_argument(
        "--out-csv",
        type=Path,
        default=Path("summaries") / "binary_sequential_best_worst.csv",
