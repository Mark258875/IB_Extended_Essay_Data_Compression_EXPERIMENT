...
def extract_distribution(dataset: str) -> str:
    ...
    Examples: 'p0.01_random', 'p0.05_random', etc.
...
def read_binary_rows(csv_paths: List[Path], debug: bool = False) -> List[Dict[str, Any]]:
    ...
    if debug:
        ...
        if rows_seen > 0 and matched_binary == 0:
            print("[hint] No 'binary' detected. Adjust is_binary_dataset() or paths.")
...
\centering
\caption{Binary random (IID) experiment: best/worst per distribution (ratio, \% over entropy, and speed).}
\label{tab:binary_random_best_worst}
...
def main():
    ap = argparse.ArgumentParser(
        description="Summarize Binary RANDOM (IID) experiment: best/worst ratio (with % over entropy) and speed per distribution."
    )

    # HARD-CODED DEFAULTS for your layout:
    ap.add_argument(
        "--brotli-csv",
        type=Path,
        default=Path("results") / "random" / "results_brotli.csv",
        help="Path to Brotli CSV (default: results/random/results_brotli.csv)",
    )
    ap.add_argument(
        "--lz4-csv",
        type=Path,
        default=Path("results") / "random" / "results_lz4.csv",
        help="Path to LZ4 CSV (default: results/random/results_lz4.csv)",
    )
    ap.add_argument(
        "--out-csv",
        type=Path,
        default=Path("summaries") / "binary_random_best_worst.csv",
