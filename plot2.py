...
def quality_cmap_norm(qualities: List[int], cmap_name: str = "cividis"):
    ...
    """
    # New-style colormap API (avoids deprecation warning)
...
class Row:
    ...
    def _compute_bpbit(kind: str, comp_bytes: float, orig_bytes: float) -> float:
        ...
        if kind == "ascii01":
            ...
            return (comp_bytes * 8) / orig_bytes
        elif kind == "bitpack":
...
def plot_all_qualities(
    ...
    colors = palette_for_qualities(qualities)  # keep multiple colors for qualities

    fig, ax = plt.subplots(figsize=(9, 6), dpi=120)

    xs = p_grid(800)
    ys = [binary_entropy(p) for p in xs]
    # Binary entropy line in navy
    ax.plot(
        xs,
        ys,
        linewidth=2.0,
        color="navy",
        label="Binary entropy H(p) (i.i.d. bound)",
        zorder=1,
    )

    # Points: color by quality (multiple colors)
    for q in qualities:
    ...
    # Legend: qualities in their respective colors
    qual_handles = [
        Line2D(
            [0],
            [0],
...
def plot_per_quality(rows: List[Row], alg: str, save_dir: Path) -> None:
    ...
    for q, sub in sorted(by_q.items()):
        ...
        k = 0
        # We only use bitpack now; keep colors dict simple
        colors = {"bitpack": "red", "other": "red"}
        ...
        for r in sorted(sub, key=lambda x: x.p):
            ...
            ax.scatter(
                ...
                y_meas,
                s=55,
                color=colors.get(r.kind, "red"),
                marker=marker,
                zorder=3,
            )

            # arrow showing gap to H(p) (also red)
            ax.annotate(
                "",
                xy=(r.p, y_meas),
                xytext=(r.p, H),
        ...
        from matplotlib.lines import Line2D
        # Only bitpack in legend now (no ascii01 -> no KeyError)
        kind_handles = [
            Line2D(
                [0],
                [0],
                marker=markers_for_kind("bitpack"),
                linestyle="none",
                markerfacecolor="red",
                markeredgecolor="none",
                label="bitpack",
...
def plot_entropy_vs_bpb(rows: List[Row], alg: str, save_dir: Path, order_label: str) -> None:
    ...
    # Scatter: color by quality, marker by kind (kept as before)
    for r in use:
    ...
    kind_handles = [
        ...
        Line2D(
            [0],
            [0],
            marker=markers_for_kind("ascii01"),
            linestyle="none",
            markerfacecolor="black",
            markeredgecolor="none",
            label="ascii01",
...
def main():
    ...
    for alg, csv_path in alg_filters:
        ...
        # keep only our binary datasets AND only bitpack
        rows = [r for r in rows if r.alg == alg and r.kind == "bitpack"]
        ...
        plot_all_qualities(
            ...
            alg,
            out_dir,
            kind_filter=None if args.first_kind == "all" else args.first_kind,
        )
